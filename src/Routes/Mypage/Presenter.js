...
const Presenter = ({ data_challenge, data_my, loading, tabs }) =>
  loading ? (
    ...
    <>
      {console.log(data_challenge)}
      <Helmet>
        <title>Mypage | Dowith</title>
      </Helmet>
      <div style={{ overflowX: "hidden" }}>
        <Section1>
          ...
          <h3>
            <strong>{data_my.nickname}</strong>님
          </h3>
          <H4 to="/mypage/edit">프로필 수정</H4>
          <CoinBox>
            <CoinImg />
            <div>
              <span>내 포인트</span>
        ...
        <Section2>
          <ButtonBox>
            <Button
              active={tabs.currentIdx === 0}
              onClick={() => {
                tabs.changeItem(0);
              }}
            >
              챌린지 목록 ({data_challenge.gathering_ongoing_count})
            </Button>
            <Button
              active={tabs.currentIdx === 1}
              onClick={() => {
                tabs.changeItem(1);
              }}
            >
              완료된 챌린지 ({data_challenge.finished_count})
            </Button>
          </ButtonBox>
          {tabs.currentItem}
        </Section2>
